...
import androidx.compose.ui.unit.dp
import com.veles.purchase.presentation.compose.DismissDirection.EndToStart
 ...
 *
 * This is the original custom swipe implementation used in the app,
 * providing exact swipe behavior with configurable thresholds.
 */
...
open class SwipeableState<T>(
     ...
     * . Fling with settling [swipeable] will always consume all the velocity provided
...
class DismissState(
    ...
    suspend fun dismiss(direction: DismissDirection) {
        val targetValue =
            if (direction == StartToEnd) DismissValue.DismissedToEnd else DismissValue.DismissedToStart
...
fun SwipeToDismiss(
    ...
    if (StartToEnd in directions) anchors += width to DismissValue.DismissedToEnd
    if (EndToStart in directions) anchors += -width to DismissValue.DismissedToStart
